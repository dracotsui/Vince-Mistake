...
### Second code 
For the second code, we have tried create two new functions which are get_player_move() and get_computer_move() functions. This code is kinda similar as before but not computer VS computer. In this code we would like to change it as person VS computer. and here are the data of how these function made:

### Player movement
the `get_player_move(board)` function prompts the iser to uper their move and check if the chosen location on the board is available. At the beginning the player have to fill in the the number in the range of 0,1,2 for row and column in terminial:
```python
row = int(input("Enter row: "))
col = int(input("Enter col: "))
```
If the board position is taken the terminal will show the as below:
```python
else:
...
### Computer movement
`get_computer_move(board, player)` actually similar as above `get_player_move(board)` function, the computer choose position by random selection:
```python
...
## Explanation
For the three possible results -- win, draw or lost, we write the details: the code about configurations of the chess piece. The Python need to check each column, row and diagonal to see whether the rival can make three consecutive spots. Based on this logic, our VScode shows these combinations for win, draw,lost game. About different configurations, it mainly about the probabilities. In each segment on the board, when the game starts, the computer have to calculate the probabilities of place the opponent will set. After realising the behind logic, we consider the probability of the first player winning Tic-Tac-Toe against itself a number of times.
